...
- *binary_variables.csv* - Contains the marginal distributions for binary variables including: Variable Name, Mean, Number of Missing Observations.
- *categorical_variables.csv* Contains the marginal distributions for categorical variables including: Variable Name & Category Name and the Number of Observations in Each Category, **NB** Missing Observations are coded as a separate category labelled missing.
- *continuous_variables.csv* - Contains the marginal distributions for continuous variables including: Variable Name, Transformed Mean, Transformed Standard Deviation, Number of Missing Observations and the Number of Decimal Places.
- *continuous_quantiles.csv* - Contains the Quantile mapping to allow for back transformation. For each continuous variable this contains the original quantile value, the transformed quantile value and an epsilon value to indicate the amount of thinning applied.
- *summary.csv* - Contains and overall summary of the dataset including:
the Number of Rows, Columns and Variable Names (for validation).
